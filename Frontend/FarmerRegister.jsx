...
export default function FarmerRegister() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setLoading(true);
    const res = await apiCall('/auth/register/farmer', 'POST', new FormData(e.target), true);
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', background: '#fff', borderRadius: 20, padding: 32, boxShadow: '0 8px 32px rgba(0,0,0,0.08)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <h2 style={{ fontWeight: 700, fontSize: '1.5rem', color: '#1f2937' }}>Farmer Registration</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div><label style={labelStyle}>Age *</label>
              <input style={inputStyle} type="number" name="age" required min="18" max="100" placeholder="Age" /></div>
          ...
          <div><label style={labelStyle}>Government License Number *</label>
            <input style={inputStyle} type="text" name="license_number" required placeholder="Enter license number" /></div>

          <div><label style={labelStyle}>Farming Category *</label>
            <select style={inputStyle} name="farming_category" required>
              <option value="">Select Category</option>
              <option value="millets">Millets Grower</option>
              <option value="vegetables">Vegetables/Fruits Grower</option>
              <option value="herbs">Cosmetic Herbs Grower</option>
            </select></div>
          ...
          <button type="submit" disabled={loading} style={{ background: `linear-gradient(135deg,${P},#2d5016)`, color: '#fff', border: 'none', padding: '16px', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Poppins, sans-serif', opacity: loading ? 0.7 : 1 }}>
